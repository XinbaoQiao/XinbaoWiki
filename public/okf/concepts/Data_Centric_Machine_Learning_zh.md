...
本页解释为什么 [数据选择](./Data_Selection_zh.md)、[样本选择偏差](./Sample_Selection_Bias_zh.md)、[合成数据](./Synthetic_Data_zh.md)、[机器遗忘](./Machine_Unlearning_zh.md) 和 [协作评估](./Collaborative_Evaluation_zh.md) 属于同一研究图谱。它们都在追问：当数据过程发生变化时，模型行为如何变化，哪些数据重要，哪些数据会伤害可靠性，以及哪些数据可以在现实成本约束下被忽略。
