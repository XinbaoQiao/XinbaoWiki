...
- **隐私边界：** Xinbaopedia 自有 Redis 与日志不保存原始问题、聊天历史、原始 IP、原始访客坐标、系统提示、私有语气配置或 API key。主页访问地图只保留全历史签名浏览器的 HLL 聚合，以及经过量化和阈值保护的地图单元；浏览器可在 `/site-activity-preferences/` 自助排除，且更换地区不会绕过该设置。托管平台仍会按其政策处理请求 IP。当前聊天消息仍会发送给配置的模型服务。详见[访问地图数据契约](docs/site-activity/README.md)。
