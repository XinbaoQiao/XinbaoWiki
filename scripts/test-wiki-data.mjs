...
assert.match(styles, /\.wiki-portal-name-button \{[\s\S]*appearance: none;[\s\S]*-webkit-appearance: none;[\s\S]*width: min\(620px, 86vw\);[\s\S]*aspect-ratio: 4 \/ 1;[\s\S]*font: inherit;[\s\S]*cursor: pointer;[\s\S]*user-select: none;[\s\S]*-webkit-tap-highlight-color: transparent;[\s\S]*\}/, 'homepage signature name uses the same display box for text and logo themes without native button chrome');
...
assert.match(styles, /\.wiki-portal-name-logo \{[\s\S]*display: none;[\s\S]*width: 100%;[\s\S]*aspect-ratio: 4 \/ 1;[\s\S]*height: 100%;[\s\S]*border: 0;[\s\S]*background: transparent;[\s\S]*box-shadow: none;[\s\S]*object-fit: contain;[\s\S]*outline: 0;[\s\S]*pointer-events: none;[\s\S]*\}/, 'homepage logo wordmarks fill the same borderless display box before being shown by the active theme');
...
assert.match(styles, /\.wiki-portal-block \{[\s\S]*--portal-section-accent: #36c;[\s\S]*border-top: 2px solid var\(--portal-section-accent\);[\s\S]*\}/, 'homepage browse top-level sections get a stronger taxonomy accent');
...
assert.match(styles, /\.wiki-portal-block h3 \{[\s\S]*font-family: var\(--font-sans\);[\s\S]*font-size: 15px;[\s\S]*font-style: normal;[\s\S]*font-weight: 700;[\s\S]*letter-spacing: 0;[\s\S]*\}/, 'homepage browse top-level headings use compact non-italic taxonomy labels');
...
assert.match(styles, /\.wiki-portal-block h3 span \{[\s\S]*padding-bottom: 1px;[\s\S]*color: color-mix\(in srgb, var\(--portal-section-accent\) 26%, var\(--wiki-text\)\);[\s\S]*\}/, 'homepage browse top-level heading text keeps a restrained accent tint');
assert.match(styles, /\.wiki-portal-block h3::after \{[\s\S]*width: 46px;[\s\S]*background: var\(--portal-section-accent\);[\s\S]*\}/, 'homepage browse top-level headings keep a compact accent rule instead of numbers');
