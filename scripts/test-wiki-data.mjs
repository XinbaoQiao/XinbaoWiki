...
assert.match(styles, /@media \(max-width: 720px\) \{[\s\S]*\.wiki-portal-grid-cube\[data-active-face\] \.wiki-portal-block \{\s*display: block;\s*opacity: 1;\s*visibility: visible/, "mobile cube mode restores all unfolded faces even after keyboard activation");
...
assert.match(styles, /\.wiki-portal-grid-cube\.wiki-portal-cube-reader-shell \.wiki-portal-cube-reader \.wiki-portal-group,[\s\S]*border-color: transparent;[\s\S]*box-shadow: none/, "the transparent cube hit map cannot draw duplicate group separators across the active face");
