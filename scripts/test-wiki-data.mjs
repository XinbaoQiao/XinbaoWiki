...
assert.match(styles, /\.wiki-portal-cube-pin \{[\s\S]*position: absolute[\s\S]*top: 42px[\s\S]*left: calc\(50% \+ 184px\)[\s\S]*min-height: 44px[\s\S]*border: 0;[\s\S]*border-radius: 0;[\s\S]*background: transparent;[\s\S]*box-shadow: none;[\s\S]*\.wiki-portal-cube-pin-icon \{[\s\S]*filter: drop-shadow[\s\S]*rotate\(-18deg\)[\s\S]*\.wiki-portal-cube-pin\[aria-pressed="true"\] \{\s*background: transparent;\s*box-shadow: none;\s*\}[\s\S]*\.wiki-portal-cube-pin\[aria-pressed="true"\] \.wiki-portal-cube-pin-icon \{[\s\S]*translateY\(5px\)[\s\S]*transition: none;/, "the chrome-free Pin preserves a practical hit target while only its physical thumbtack casts a shadow and presses into the paper");
