...
assert.match(homepagePortal, /onPointerLeave=\{\(\) => \{[\s\S]*clearCubeHoverIntent\(\);[\s\S]*setActiveCubeFace\(null\)/, "leaving the cube cancels pending hover intent and restores the resting pose");
...
assert.match(homepagePortal, /className="wiki-portal-cube-pin"[\s\S]*>\s*<span aria-hidden="true" \/>\s*Pin\s*<\/button>/, "the visible cube pin control keeps the concise label Pin in every language and state");
...
assert.match(homepagePortal, /hoveredCubeItemHref[\s\S]*data-cube-item-active=\{!isReader && hoveredCubeItemHref === item\.href[\s\S]*onPointerEnter=\{isReader \? \(\) => setHoveredCubeItemHref\(item\.href\)/, "the transparent hit layer mirrors item hover back onto the one visible original face");
assert.match(homepagePortal, /wiki-portal-cube-reader-shell[\s\S]*renderPortalSectionContent\(activeCubeSection,[\s\S]*true\)/, "the active face keeps a flat anchor hit layer so every transformed face has reliable navigation");
assert.match(styles, /\.wiki-portal-grid-cube\.wiki-portal-cube-reader-shell \.wiki-portal-cube-reader \{[\s\S]*border-color: transparent[\s\S]*background: transparent[\s\S]*box-shadow: none[\s\S]*pointer-events: auto[\s\S]*transform: none/, "the hit layer accepts pointer events without drawing an additional page surface");
assert.match(styles, /\.wiki-portal-cube-reader h3 \{[\s\S]*opacity: 0[\s\S]*\.wiki-portal-cube-reader a,[\s\S]*color: transparent/, "duplicate hit-layer typography stays invisible so only the original cube face is displayed");
assert.match(styles, /\.wiki-portal-cube-pin \{[\s\S]*position: absolute[\s\S]*top: 20px[\s\S]*right: 22px[\s\S]*\.wiki-portal-cube-pin\[aria-pressed="true"\]/, "the compact Pin control stays in the active face's upper-right corner and has a visible pinned state");
...
assert.ok(styles.includes(".wiki-portal-cube-panel::before") && styles.includes(".wiki-portal-cube-face-front::after") && styles.includes("box-shadow: 0 0 11px"), "typographic cube builds its paper bevel and shared illuminated seam in CSS");
assert.ok(styles.includes(".wiki-portal-block a::after") && styles.includes("content: '↗'") && styles.includes("translateX(3px) translateZ(18px)"), "typographic cube links reveal a directional lift interaction without instructional copy");
...
assert.ok(styles.includes("[data-active-face=\"top\"]") && styles.includes("rotateX(-90deg) rotateY(0deg)") && styles.includes("[data-active-face=\"right\"]") && styles.includes("rotateX(0deg) rotateY(-90deg)"), "hover and focus rotate each content face into a head-on reading position");
...
assert.ok(!homepagePortal.includes('cubeTextureWords') && !styles.includes('.wiki-portal-cube-texture') && styles.includes("--portal-cube-paper:") && /\.wiki-portal-grid-cube\[data-active-face\] \.wiki-portal-block \{\s*opacity: 0/.test(styles), "opaque paper-like faces prevent mirrored typography while inactive faces and their seams disappear in reading mode");
...
assert.match(styles, /\.wiki-portal-grid-cube \.wiki-portal-group-label \{[\s\S]*border-left: 3px solid[\s\S]*font-size: 12px[\s\S]*font-weight: 820[\s\S]*letter-spacing: \.075em/, 'cube group headings are visibly stronger than link rows and clearly own the entries below them');
assert.match(styles, /data-active-face="top"[\s\S]*data-cube-face="right"[\s\S]*\.wiki-portal-group-label \{[\s\S]*background:[\s\S]*box-shadow:[\s\S]*transform: translateZ\(16px\)/, 'every active cube face gives its group headings an unmistakable theme-aware lift');
assert.match(styles, /\.wiki-portal-grid-cube \.wiki-portal-cube-face-top a,[\s\S]*wiki-portal-cube-face-front a,[\s\S]*wiki-portal-cube-face-right a,[\s\S]*data-active-face="right"[\s\S]*padding-block: 3px/, 'all three reading faces preserve their final rows without weakening the typography scale');
