...
assert.match(styles, /--font-display: "Cormorant Garamond"/, 'homepage display typography follows the preview font direction');
assert.match(styles, /\.wiki-portal h1 \{[\s\S]*font-family: var\(--font-display\);[\s\S]*font-style: italic;[\s\S]*\}/, 'Xinbaopedia uses the preview-inspired display face');
assert.match(styles, /\.wiki-portal-owner \{[\s\S]*font-family: var\(--font-display\);[\s\S]*font-size: 46px;[\s\S]*\}/, 'homepage shows the owner name in a preview-inspired art type treatment');
assert.match(styles, /\.wiki-portal-fields \{[\s\S]*max-width: 560px;[\s\S]*font-size: 13px;[\s\S]*\}/, 'research fields style block stays scoped to the homepage');
...
assert.match(styles, /\.wiki-portal-count \{[\s\S]*max-width: 640px;[\s\S]*text-align: left;[\s\S]*\}/, 'homepage entry counts align with the search box width');
...
assert.match(homePage, /\/xinbaopedia-icon\.png/, 'homepage uses the new PNG site icon');
assert.match(homePage, /className="wiki-portal-owner"[\s\S]*Xinbao Qiao/, 'homepage shows the owner name in the masthead');
assert.match(homePage, /AI for Networks · Data-centric Machine Learning · Federated Learning/, 'homepage lists research fields under the owner name');
assert.doesNotMatch(homePage, /Featured entry|wiki-portal-featured|\/images\/Portrait\.png|The academic wiki of Xinbao Qiao/, 'homepage removes the featured-entry block, portrait, and old tagline');
