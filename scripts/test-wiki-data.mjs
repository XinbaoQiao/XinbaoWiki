...
assert.match(homepagePortal, /wiki-portal-cube-reader-shell[\s\S]*renderPortalSectionContent\(activeCubeSection/, "the active face uses a flat reading layer so every transformed face has reliable link hit testing");
assert.match(styles, /\.wiki-portal-grid-cube\.wiki-portal-cube-reader-shell[\s\S]*pointer-events: none[\s\S]*\.wiki-portal-cube-reader \{[\s\S]*pointer-events: auto[\s\S]*transform: none/, "the flat cube reader alone accepts item hover and clicks after face rotation");
