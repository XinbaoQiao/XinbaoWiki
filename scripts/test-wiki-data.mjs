...
assert.match(styles, /\.wiki-portal-directory\[open\] summary span::after \{[\s\S]*content: "▾";[\s\S]*\}/, 'homepage browse disclosure points downward when expanded');
...
assert.match(homepagePortal, /const milestoneLabels[\s\S]*5 milestones[\s\S]*Milestones[\s\S]*5 项里程碑[\s\S]*里程碑/, 'homepage milestones have compact bilingual labels and an honest item count');
assert.match(homepagePortal, /const milestoneEntries[\s\S]*ICML 2026 paper accepted[\s\S]*Completed master’s degree[\s\S]*AAAI 2026 paper accepted[\s\S]*Started full-time research internship[\s\S]*Two ICLR 2025 papers accepted/, 'homepage keeps five high-signal English milestones');
assert.match(homepagePortal, /ICML 2026 论文录用[\s\S]*完成硕士学位[\s\S]*AAAI 2026 论文录用[\s\S]*开始全职研究实习[\s\S]*两篇 ICLR 2025 论文录用/, 'homepage keeps the five Chinese milestone counterparts');
assert.match(homepagePortal, /dateTime: '2026-04'[\s\S]*dateTime: '2025-12'[\s\S]*dateTime: '2025-11'[\s\S]*dateTime: '2025-06'[\s\S]*dateTime: '2025-01'/, 'homepage milestones include year-and-month machine-readable dates in reverse chronology');
assert.match(homepagePortal, /const updateEntries[\s\S]*\.\.\.milestoneEntries\.en[\s\S]*dateTime: '2023-03'[\s\S]*dateTime: '2022-09'[\s\S]*dateTime: '2022-07'[\s\S]*\.\.\.milestoneEntries\.zh/, 'homepage update archive retains milestones and older verified events in reverse chronology');
...
assert.doesNotMatch(homepagePortal, /Research code released|研究代码公开|Academic service|学术服务|Serving as a reviewer/, 'homepage milestones exclude routine code and service updates');
assert.match(homepagePortal, /className="wiki-portal-disclosures"[\s\S]*className="wiki-portal-news wiki-portal-timeline"[\s\S]*className="wiki-portal-milestones wiki-portal-timeline"[\s\S]*className="wiki-portal-directory"/, 'homepage places Updates, Milestones, and Browse as sibling disclosure sections');
assert.match(styles, /\.wiki-portal-disclosures \{[\s\S]*--portal-search-leading-width: 50px;[\s\S]*--portal-search-submit-width: 96px;[\s\S]*max-width: 690px;/, 'homepage disclosures share the portal search width and control measurements');
assert.match(styles, /\.wiki-portal-timeline \{[\s\S]*width: calc\(100% - var\(--portal-search-leading-width\) - var\(--portal-search-submit-width\)\);[\s\S]*margin-right: var\(--portal-search-submit-width\);[\s\S]*margin-left: var\(--portal-search-leading-width\);[\s\S]*border-left: 4px solid var\(--site-theme-accent\);[\s\S]*border-radius: 2px;[\s\S]*background: color-mix/, 'homepage timelines align to the white search field while excluding the Search button');
...
assert.match(styles, /\.wiki-portal-news-list \{[\s\S]*grid-template-columns: 1fr;[\s\S]*padding: 0 12px 4px;/, 'homepage milestones use one compact reading column at every breakpoint');
assert.match(styles, /\.wiki-portal-news-list li \{[\s\S]*grid-template-columns: 80px minmax\(0, 1fr\);[\s\S]*align-items: start;[\s\S]*gap: 10px;[\s\S]*padding: 7px 0;/, 'homepage milestone dates and headlines share a compact stable first-line alignment');
assert.match(styles, /\.wiki-portal-news-list time \{[\s\S]*padding-top: 2px;[\s\S]*font-size: 11px;[\s\S]*line-height: 1\.35;[\s\S]*\}[\s\S]*\.wiki-portal-news-list a \{[\s\S]*display: block;[\s\S]*font-size: 14px;[\s\S]*line-height: 1\.35;[\s\S]*text-wrap: balance;[\s\S]*\}[\s\S]*\.wiki-portal-news-list p \{[\s\S]*font-size: 12\.5px;[\s\S]*line-height: 1\.4;[\s\S]*text-wrap: balance;/, 'homepage milestones retain readable typography and avoid orphaned short final lines');
assert.match(styles, /\.wiki-portal-directory summary \{[\s\S]*width: min\(100%, 620px\);[\s\S]*margin-inline: auto;/, 'homepage Browse control remains independently centered');
...
assert.match(homepagePortal, /const \[milestonesOpen, setMilestonesOpen\] = useState\(false\);/, 'homepage Milestones is collapsed by default');
assert.match(homepagePortal, /const collapsibleSections = \{ browse: browseOpen, milestones: milestonesOpen, news: newsOpen \};[\s\S]*const allSectionsClosed = Object\.values\(collapsibleSections\)\.every\(\(open\) => !open\);[\s\S]*wiki-portal-collapsed/, 'homepage computes collapsed state across all three peer disclosures');
assert.match(homepagePortal, /const expandAllSections = \(\) => \{[\s\S]*setBrowseOpen\(true\);[\s\S]*setMilestonesOpen\(true\);[\s\S]*setNewsOpen\(true\);[\s\S]*\};[\s\S]*const collapseAllSections = \(\) => \{[\s\S]*setBrowseOpen\(false\);[\s\S]*setMilestonesOpen\(false\);[\s\S]*setNewsOpen\(false\);[\s\S]*\};/, 'homepage signature button expands or collapses Updates, Milestones, and Browse together');
...
assert.match(homepagePortal, /<details[\s\S]*className="wiki-portal-milestones wiki-portal-timeline"[\s\S]*id="portal-milestones"[\s\S]*open=\{milestonesOpen\}[\s\S]*<summary[\s\S]*event\.preventDefault\(\);[\s\S]*setMilestonesOpen\(\(open\) => !open\);/, 'homepage Milestones uses native disclosure semantics with race-free controlled state');
