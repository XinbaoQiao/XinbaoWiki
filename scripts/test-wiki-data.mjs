...
assert.doesNotMatch(layout, /text: pathWithBasePath\('\/xinbaopedia-icon\.png'\)/, 'layout removes the redundant text-theme favicon entry');
...
assert.match(languageToggle, /type SitePaletteName = 'blue' \| 'gold' \| 'rose' \| 'green' \| 'violet' \| 'charcoal';/, 'site palette exposes six visually distinct color themes');
assert.doesNotMatch(languageToggle, /mode: 'text'|Text wordmark theme|option\.mode === 'text'/, 'site palette removes the text theme that duplicated blue accents and charcoal branding');
assert.match(languageToggle, /const sitePaletteOptions: SitePaletteOption\[\] = \[[\s\S]*mode: 'blue'[\s\S]*mode: 'gold'[\s\S]*mode: 'rose'[\s\S]*mode: 'green'[\s\S]*mode: 'violet'[\s\S]*mode: 'charcoal'[\s\S]*mode: 'auto'[\s\S]*\];/, 'site palette orders six manual colors before auto mode');
assert.match(languageToggle, /function sitePaletteForLocalTime\(date = new Date\(\)\): SitePaletteName \{[\s\S]*hour >= 5 && hour < 9[\s\S]*return 'blue'[\s\S]*hour >= 9 && hour < 13[\s\S]*return 'gold'[\s\S]*hour >= 13 && hour < 16[\s\S]*return 'rose'[\s\S]*hour >= 16 && hour < 19[\s\S]*return 'green'[\s\S]*hour >= 19 && hour < 22[\s\S]*return 'violet'[\s\S]*return 'charcoal'[\s\S]*\}/, 'auto mode rotates through all six color themes by local time');
...
assert.doesNotMatch(styles, /data-site-palette="text"|\.site-palette-text/, 'site palette CSS removes the duplicate text theme');
assert.match(styles, /@media \(hover: hover\) and \(pointer: fine\) \{[\s\S]*\.site-palette-switcher \{[\s\S]*width: 30px;[\s\S]*\.site-palette-switcher:hover,[\s\S]*\.site-palette-switcher:focus-within \{[\s\S]*width: 174px;/, 'fine-pointer palette expands far enough for six colors plus auto mode');
