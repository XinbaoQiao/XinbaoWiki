...
assert.match(styles, /\.wiki-portal-name-button \{[\s\S]*appearance: none;[\s\S]*width: min\(620px, 86vw\);[\s\S]*aspect-ratio: 4 \/ 1;[\s\S]*font: inherit;[\s\S]*cursor: pointer;[\s\S]*user-select: none;[\s\S]*\}/, 'homepage signature name uses the same display box for text and logo themes');
...
assert.match(styles, /\.wiki-portal-name-logo \{[\s\S]*display: none;[\s\S]*width: 100%;[\s\S]*aspect-ratio: 4 \/ 1;[\s\S]*height: 100%;[\s\S]*border: 0;[\s\S]*background: transparent;[\s\S]*box-shadow: none;[\s\S]*object-fit: contain;[\s\S]*\}/, 'homepage logo wordmarks fill the same borderless display box before being shown by the active theme');
