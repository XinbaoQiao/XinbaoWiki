...
assert.match(homepagePortal, /const newsLabels[\s\S]*5 milestones[\s\S]*Latest Updates[\s\S]*5 项里程碑[\s\S]*最新动态/, 'homepage milestones have compact bilingual labels and an honest item count');
assert.match(homepagePortal, /const newsEntries[\s\S]*ICML 2026 paper accepted[\s\S]*Completed master’s degree[\s\S]*AAAI 2026 paper accepted[\s\S]*Started full-time research internship[\s\S]*Two ICLR 2025 papers accepted/, 'homepage keeps five high-signal English milestones');
...
assert.match(homepagePortal, /newsEntries\[language\]\.slice\(0, 6\)\.map/, 'homepage retains a hard upper limit of six milestones');
...
assert.match(styles, /\.wiki-portal-news \{[\s\S]*width: calc\(100% - var\(--portal-search-leading-width\) - var\(--portal-search-submit-width\)\);[\s\S]*margin-right: var\(--portal-search-submit-width\);[\s\S]*margin-left: var\(--portal-search-leading-width\);[\s\S]*border-left: 4px solid var\(--site-theme-accent\);[\s\S]*border-radius: 2px;[\s\S]*background: color-mix/, 'homepage milestones align to the white search field while excluding the Search button');
assert.doesNotMatch(styles, /\.wiki-portal-news \{[\s\S]*radial-gradient|\.wiki-portal-news \{[\s\S]*box-shadow:/, 'homepage milestones avoid the former wide promotional-card effects');
...
assert.match(styles, /@media \(max-width: 720px\) \{[\s\S]*\.wiki-portal-news \{[\s\S]*width: 100%;[\s\S]*margin-inline: 0;/, 'homepage milestones use the available width on small screens instead of inheriting narrow desktop search controls');
