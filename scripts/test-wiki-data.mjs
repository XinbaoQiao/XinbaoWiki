...
assert.match(homepagePortal, /browseView === 'cube' && !activeCubeFace[\s\S]*wiki-portal-cube-hover-zones[\s\S]*cubeFaceNames\.map\(\(face\)[\s\S]*data-cube-hover-face=\{face\}[\s\S]*onPointerEnter=\{\(\) => scheduleCubeFace\(face\)\}[\s\S]*onPointerLeave=\{clearCubeHoverIntent\}/, "stable two-dimensional hover zones activate top, front, and right only while the cube is resting");
...
assert.match(styles, /@media \(max-width: 720px\) \{[\s\S]*\.wiki-portal-cube-hover-zones \{\s*display: none;/, "mobile unfolded cube panels omit desktop-only pointer projection zones");
