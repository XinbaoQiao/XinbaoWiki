...
assert.ok(!homepagePortal.includes('onPointerOut=') && styles.includes('width: var(--portal-cube-reading-width)') && styles.includes('margin-inline: auto'), "the stable centered Browse-width cube stage, rather than moving faces, owns pointer reset behavior");
...
assert.match(styles, /\.wiki-portal-cube-pin \{[\s\S]*position: absolute[\s\S]*top: calc\(var\(--portal-cube-reading-top\) - var\(--portal-cube-pin-height\)\)[\s\S]*right: calc\(\(100% - var\(--portal-cube-reading-width\)\) \/ 2\)[\s\S]*left: auto[\s\S]*min-height: var\(--portal-cube-pin-height\)[\s\S]*border: 0;[\s\S]*border-radius: 0;[\s\S]*background: transparent;[\s\S]*box-shadow: none;[\s\S]*\.wiki-portal-cube-pin-icon \{[\s\S]*filter: drop-shadow[\s\S]*rotate\(-18deg\)[\s\S]*\.wiki-portal-cube-pin\[aria-pressed="true"\] \{\s*background: transparent;\s*box-shadow: none;\s*\}[\s\S]*\.wiki-portal-cube-pin\[aria-pressed="true"\] \.wiki-portal-cube-pin-icon \{[\s\S]*translateY\(5px\)[\s\S]*transition: none;/, "the chrome-free Pin derives both axes from the shared reading-paper geometry while only its physical thumbtack casts a shadow and presses into the paper");
...
assert.match(styles, /\.wiki-portal-cube-stage-cube \{[\s\S]*--portal-cube-reading-width: min\(100%, 920px\);[\s\S]*--portal-cube-reading-height: 620px;[\s\S]*--portal-cube-reading-top: 32px;[\s\S]*--portal-cube-pin-height: 44px;[\s\S]*width: var\(--portal-cube-reading-width\);[\s\S]*\.wiki-portal-grid-cube\[data-active-face\] \{[\s\S]*--portal-cube-width: var\(--portal-cube-reading-width\);[\s\S]*--portal-cube-height: var\(--portal-cube-reading-height\);[\s\S]*top: var\(--portal-cube-reading-top\);/, "every desktop reading face and its Pin share one 920px Browse-directory track and one vertical geometry contract");
