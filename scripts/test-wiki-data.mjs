...
assert.match(rootReadme, /<a id="english"><\/a>[\s\S]*## English[\s\S]*### See the product work[\s\S]*xinbaopedia-tour-en\.gif[\s\S]*From homepage to knowledge product[\s\S]*<a id="simplified-chinese"><\/a>/, 'README keeps the English walkthrough inside the English section');
assert.match(rootReadme, /<a href="https:\/\/xinbaopedia\.top\/"><img src="public\/readme\/xinbaopedia-tour-en\.gif"/, 'English README walkthrough opens the live homepage represented by its first three states');
assert.match(rootReadme, /alt="Animated English walkthrough[\s\S]*public Site activity map[\s\S]*Browse Xinbaopedia knowledge cube[\s\S]*citation-grounded AI answer/, 'English README walkthrough names its current activity, browse, and grounded-answer states');
assert.doesNotMatch(rootReadme.slice(rootReadme.indexOf('<a id="english"></a>'), rootReadme.indexOf('<a id="simplified-chinese"></a>')), /href="https:\/\/xinbaopedia\.top\/wiki\/(?:DynFrs|When_Sample_Selection_Bias_Precipitates_Model_Collapse)\/"[\s\S]*xinbaopedia-tour-en\.gif/, 'English README walkthrough no longer sends the homepage-focused tour to a single paper');
assert.match(rootReadme, /<a id="simplified-chinese"><\/a>[\s\S]*## 简体中文[\s\S]*### 看见产品如何工作[\s\S]*xinbaopedia-tour-zh\.gif[\s\S]*从主页，到知识产品/, 'README keeps the Chinese walkthrough inside the Chinese section');
assert.match(rootReadme, /<a href="https:\/\/xinbaopedia\.top\/"><img src="public\/readme\/xinbaopedia-tour-zh\.gif"/, 'Chinese README walkthrough opens the live homepage represented by its first three states');
assert.match(rootReadme, /alt="Xinbaopedia 中文动图演示：[\s\S]*公开访问足迹[\s\S]*浏览 Xinbaopedia 知识魔方[\s\S]*带引用来源的 AI 回答/, 'Chinese README walkthrough names its current activity, browse, and grounded-answer states');
...
assert.match(rootReadme, /<a id="english"><\/a>[\s\S]*From homepage to knowledge product[\s\S]*<a id="simplified-chinese"><\/a>[\s\S]*从主页，到知识产品/, 'README contains complete English and Chinese product sections on one page');
assert.match(rootReadme, /lifetime Site activity in a thresholded world view[\s\S]*全部历史访问足迹/, 'README explains the public all-history activity view in both languages');
...
assert.ok(!homepagePortal.includes('onPointerOut=') && styles.includes('width: min(100%, 920px)') && styles.includes('margin-inline: auto'), "the stable full-width cube stage, rather than moving faces, owns pointer reset behavior while only the selected paper joins the Updates track");
assert.match(homepagePortal, /className=\{'wiki-portal-cube-stage wiki-portal-cube-stage-' \+ browseView\}[\s\S]*data-active-face=\{browseView === 'cube' \? activeCubeFace \?\? undefined : undefined\}[\s\S]*data-face-settled=\{browseView === 'cube' && activeCubeFace && cubeFaceSettled \? '' : undefined\}/, "the stable stage exposes the explicit settled phase used to leave the 3D perspective only after the turn completes");
assert.match(styles, /\.wiki-portal-cube-stage-cube\[data-face-settled\]\[data-active-face\] \{\s*perspective: none;/, "the stage preserves perspective through the face turn and removes it only for the settled reading surface");
...
assert.match(homepagePortal, /browseView === 'cube' && activeCubeFace[\s\S]*className="wiki-portal-cube-pin"[\s\S]*data-cube-face=\{activeCubeFace\}[\s\S]*className="wiki-portal-cube-pin-icon"/, "the stable stage exposes a visible thumbtack control for the active face");
assert.ok(!homepagePortal.includes('<span>Pin</span>'), "the thumbtack control remains icon-only while its localized aria-label carries the action name");
...
assert.match(styles, /\.wiki-portal-cube-pin \{[\s\S]*position: absolute[\s\S]*top: calc\(var\(--portal-cube-reading-top\) - var\(--portal-cube-pin-height\)\)[\s\S]*right: calc\(100% - var\(--portal-updates-track-offset\) - var\(--portal-cube-reading-width\)\)[\s\S]*left: auto[\s\S]*width: var\(--portal-cube-pin-height\);[\s\S]*min-width: var\(--portal-cube-pin-height\);[\s\S]*min-height: var\(--portal-cube-pin-height\);[\s\S]*padding: 0;[\s\S]*border: 0;[\s\S]*border-radius: 0;[\s\S]*background: transparent;[\s\S]*box-shadow: none;[\s\S]*\.wiki-portal-cube-pin-icon \{[\s\S]*filter: drop-shadow[\s\S]*rotate\(-18deg\)[\s\S]*\.wiki-portal-cube-pin\[aria-pressed="true"\] \{\s*background: transparent;\s*box-shadow: none;\s*\}[\s\S]*\.wiki-portal-cube-pin\[aria-pressed="true"\] \.wiki-portal-cube-pin-icon \{[\s\S]*translateY\(10px\)[\s\S]*transition: none;/, "the icon-only Pin uses a stable square target derived from the shared reading-paper geometry and presses its actual needle tip into the paper edge");
assert.match(styles, /\.wiki-portal-cube-pin:hover,\s*\.wiki-portal-cube-pin:focus-visible \{\s*transform: scale\(1\.03\);\s*\}/, "hover and focus enlarge the physical Pin without drawing a button border, fill, or container shadow");
assert.match(styles, /\.wiki-portal-disclosures \{[\s\S]*--portal-updates-track-width: min\([\s\S]*--portal-updates-track-offset: max\([\s\S]*\.wiki-portal-timeline \{[\s\S]*width: var\(--portal-updates-track-width\);[\s\S]*margin-left: var\(--portal-updates-track-offset\);/, "Latest Updates exposes its exact responsive width and horizontal offset as the shared alignment track");
assert.match(styles, /\.wiki-portal-cube-stage-cube \{[\s\S]*--portal-cube-reading-width: var\(--portal-updates-track-width\);[\s\S]*--portal-cube-reading-height: 620px;[\s\S]*--portal-cube-reading-top: 32px;[\s\S]*--portal-cube-pin-height: 44px;[\s\S]*width: min\(100%, 920px\);[\s\S]*\.wiki-portal-grid-cube\[data-active-face\] \{[\s\S]*--portal-cube-width: var\(--portal-cube-reading-width\);[\s\S]*--portal-cube-height: var\(--portal-cube-reading-height\);[\s\S]*margin-right: auto;[\s\S]*margin-left: var\(--portal-updates-track-offset\);[\s\S]*top: var\(--portal-cube-reading-top\);/, "every desktop reading face and its Pin use the exact Latest Updates left and right boundaries while the resting cube retains the full stage");
assert.match(styles, /\.wiki-portal-grid-cube \.wiki-portal-cube-panel \{[\s\S]*width: 100%;[\s\S]*height: 100%;/, "cube panels inherit the already-resolved grid rectangle instead of re-evaluating percentage-bearing track variables against themselves");
...
assert.match(homepagePortal, /const cubeReturnLabels[\s\S]*label: 'Back'[\s\S]*ariaLabel: 'Return to the initial cube'[\s\S]*label: '返回'[\s\S]*ariaLabel: '返回初始魔方'/, "the touch reading state exposes concise bilingual return copy");
assert.match(homepagePortal, /const returnTouchCubeToInitial = \(\) => \{[\s\S]*resetTouchCubeGesture\(\);[\s\S]*setActiveCubeFace\(null\);[\s\S]*setPinnedCubeFace\(null\);[\s\S]*setTouchCubeAngle\('front'\);[\s\S]*removeItem\('xinbaopedia-cube-pinned-face'\)/, "every explicit touch return restores the balanced cube and clears transient or inherited face state");
assert.match(homepagePortal, /className="wiki-portal-cube-touch-return"[\s\S]*onClick=\{returnTouchCubeToInitial\}[\s\S]*<svg[\s\S]*cubeReturnLabels\[language\]\.label/, "the selected touch face renders a native icon-and-label return button wired to the shared reset action");
assert.match(styles, /\.wiki-portal-cube-touch-return \{\s*display: none;[\s\S]*@media \(hover: none\), \(pointer: coarse\) \{[\s\S]*\.wiki-portal-cube-touch-return \{[\s\S]*top: 28px;[\s\S]*right: max\(12px, calc\(\(100% - 620px\) \/ 2 \+ 12px\)\);[\s\S]*min-height: 44px;/, "the return control is touch-only, practical to hit, and follows the centered phone or tablet reading paper");
...
assert.match(styles, /\.wiki-portal-grid-cube\[data-active-face\] \{[\s\S]*transform: none;[\s\S]*transform-style: preserve-3d;[\s\S]*\.wiki-portal-grid-cube\[data-face-settled\]\[data-active-face\] \{[\s\S]*transform-style: flat;/, "the active cube remains in the 3D scene while turning and becomes a flat reading surface only after it settles");
assert.ok(!homepagePortal.includes("transformStyle: 'flat'") && !homepagePortal.includes("perspective: 'none'"), "component inline styles do not flatten the compositor in the same frame that starts the CSS turn");
...
assert.match(homepagePortal, /setCubeFaceSettled\(false\)[\s\S]*matchMedia\('\(prefers-reduced-motion: reduce\)'\)[\s\S]*setCubeFaceSettled\(true\)[\s\S]*completeCubeFaceTurn[\s\S]*event\.target !== event\.currentTarget[\s\S]*event\.propertyName !== 'transform'[\s\S]*setCubeFaceSettled\(true\)[\s\S]*onTransitionEnd=\{completeCubeFaceTurn\}/, "cube links become interactive only after the renderer reports the transform transition complete, or immediately when reduced motion is requested");
assert.ok(!homepagePortal.includes('cubeTurnDurationMs') && !homepagePortal.includes('touchCubeTurnDurationMs'), "cube settling no longer races compositor progress through wall-clock timers");
...
assert.ok(!homepagePortal.includes('cubeTextureWords') && !styles.includes('.wiki-portal-cube-texture') && styles.includes("--portal-cube-paper:") && /\.wiki-portal-grid-cube\[data-face-settled\]\[data-active-face\] \.wiki-portal-block \{\s*display: none;\s*opacity: 0;\s*visibility: hidden/.test(styles), "opaque paper-like faces prevent mirrored typography while inactive faces leave the compositor once the reading state settles");
assert.match(styles, /\.wiki-portal-grid-cube\[data-face-settled\]\[data-active-face="right"\] \[data-cube-face="right"\] \{[\s\S]*display: block;[\s\S]*opacity: 1;[\s\S]*visibility: visible;[\s\S]*transform-style: flat;/, "the selected desktop face explicitly survives the higher-specificity settled cleanup and becomes the sole flat reading plane");
...
assert.match(styles.match(/\.wiki-portal-grid-cube \.wiki-portal-cube-panel \{([^}]*)\}/)?.[1] ?? '', /opacity \.24s ease/, "adjacent cube faces fade during the preserved-perspective turn instead of vanishing before motion begins");
...
assert.match(homepagePortal, /className="wiki-portal-disclosures"[\s\S]*className="wiki-portal-news wiki-portal-timeline"[\s\S]*<VisitorAtlasDisclosure[\s\S]*className="wiki-portal-directory"/, 'homepage places Updates, Site activity, and Browse as sibling disclosure sections in that order');
...
assert.match(styles, /\.wiki-portal-disclosures > details \{[\s\S]*interpolate-size: allow-keywords;[\s\S]*\}[\s\S]*\.wiki-portal-disclosures > details::details-content \{[\s\S]*block-size: 0;[\s\S]*opacity: 0;[\s\S]*transition:[\s\S]*block-size \.24s[\s\S]*opacity \.16s[\s\S]*\}[\s\S]*\.wiki-portal-disclosures > details\[open\]::details-content \{[\s\S]*block-size: auto;[\s\S]*opacity: 1;/, 'homepage disclosures animate their content smoothly in both directions');
assert.doesNotMatch(styles, /\.wiki-portal-disclosures > details::details-content \{[^}]*content-visibility/s, 'homepage disclosure animation avoids the Chromium details-content visibility deadlock');
...
assert.match(styles, /\.wiki-portal-timeline \{[\s\S]*width: var\(--portal-updates-track-width\);[\s\S]*margin-left: var\(--portal-updates-track-offset\);[\s\S]*border-left: 4px solid var\(--site-theme-accent\);[\s\S]*border-radius: 2px;[\s\S]*background: color-mix/, 'homepage Updates consumes the shared track that keeps it and fixed cube faces aligned to the white search field');
...
assert.match(homepagePortal, /const collapsibleSections = \{ activity: activityOpen, browse: browseOpen, news: newsOpen \};[\s\S]*const allSectionsClosed = Object\.values\(collapsibleSections\)\.every\(\(open\) => !open\);[\s\S]*wiki-portal-collapsed/, 'homepage computes collapsed state across all three peer disclosures');
assert.match(homepagePortal, /const expandAllSections = \(\) => \{[\s\S]*setBrowseOpen\(true\);[\s\S]*setNewsOpen\(true\);[\s\S]*setActivityOpen\(true\);[\s\S]*\};[\s\S]*const collapseAllSections = \(\) => \{[\s\S]*setBrowseOpen\(false\);[\s\S]*setNewsOpen\(false\);[\s\S]*setActivityOpen\(false\);[\s\S]*\};/, 'homepage signature button expands or collapses Updates, Site activity, and Browse together');
...
assert.match(homepagePortal, /aria-controls="portal-news portal-activity portal-directory"[\s\S]*aria-expanded=\{!allSectionsClosed\}/, 'homepage signature button names and controls all three disclosure sections');
