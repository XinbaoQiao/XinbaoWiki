...
assert.match(homepagePortal, /const collapseAllSections = \(\) => \{[\s\S]*setBrowseOpen\(false\);[\s\S]*\};[\s\S]*className="wiki-portal-name-button"[\s\S]*onClick=\{collapseAllSections\}[\s\S]*onDoubleClick=\{collapseAllSections\}/, 'homepage signature button activation collapses all current collapsible sections');
