...
assert.match(styles, /\.wiki-portal-name-text \{[\s\S]*animation: wiki-name-write 1\.65s cubic-bezier\(\.33, 0, \.2, 1\) \.12s both;[\s\S]*color: var\(--signature-ink\);[\s\S]*\}/, 'homepage name reveal uses a slower handwriting-like animation speed without image chrome');
...
assert.match(styles, /\.wiki-portal-block h3 \{[\s\S]*font-family: var\(--font-sans\);[\s\S]*font-size: 18px;[\s\S]*font-style: normal;[\s\S]*font-weight: 700;[\s\S]*letter-spacing: 0;[\s\S]*\}/, 'homepage browse top-level headings clearly outrank secondary taxonomy labels');
