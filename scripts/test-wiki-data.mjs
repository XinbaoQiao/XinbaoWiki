...
assert.match(deployProductionScript, /function configuredProxyEnv\(baseEnv\)[\s\S]*process\.env\[key\][\s\S]*env\[key\] = process\.env\[key\]/, 'deployment wrapper restores configured proxies only for authenticated staged checks');
...
assert.match(deployProductionScript, /const stagedSmokeEnv = configuredProxyEnv\(env\)[\s\S]*runStagedSmoke\(vercelCommand, stagedSmokeEnv, stagedUrl\)/, 'deployment wrapper confines configured proxies to the protected staged deployment check');
