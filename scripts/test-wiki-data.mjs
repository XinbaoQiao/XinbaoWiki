...
assert.match(styles, /\.wiki-portal-block h3 \{[\s\S]*font-style: italic;[\s\S]*font-variant-caps: small-caps;[\s\S]*\}/, 'homepage browse top-level headings use a more distinctive text shape');
assert.match(styles, /\.wiki-portal-block h3 span \{[\s\S]*transform: skewX\(-3deg\);[\s\S]*\}/, 'homepage browse top-level heading text has a subtle shaped treatment');
assert.match(styles, /\.wiki-portal-block h3::after \{[\s\S]*width: 38px;[\s\S]*background: var\(--portal-section-accent\);[\s\S]*\}/, 'homepage browse top-level headings keep a compact accent rule instead of numbers');
