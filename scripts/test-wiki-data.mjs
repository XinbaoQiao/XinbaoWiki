...
assert.match(styles, /\.wiki-portal-grid \{[\s\S]*counter-reset: portal-section;[\s\S]*align-items: start;[\s\S]*\}/, 'homepage browse directory numbers its top-level taxonomy sections');
...
assert.match(styles, /\.wiki-portal-block h3::before \{[\s\S]*content: counter\(portal-section, decimal-leading-zero\);[\s\S]*border: 1px solid var\(--portal-section-accent\);[\s\S]*\}/, 'homepage browse top-level headings show compact numbered taxonomy badges');
