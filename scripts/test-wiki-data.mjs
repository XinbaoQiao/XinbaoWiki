...
assert.match(styles, /\.wiki-portal-news \{[\s\S]*width: min\(100%, 620px\);[\s\S]*border-left: 4px solid var\(--site-theme-accent\);[\s\S]*border-radius: 2px;[\s\S]*background: color-mix/, 'homepage milestones use a narrow flat wiki-style panel');
...
assert.match(styles, /\.wiki-portal-news-list \{[\s\S]*grid-template-columns: 1fr;[\s\S]*padding: 0 16px 8px;/, 'homepage milestones use one compact reading column at every breakpoint');
assert.match(styles, /\.wiki-portal-news-list li \{[\s\S]*grid-template-columns: 90px minmax\(0, 1fr\);[\s\S]*align-items: baseline;[\s\S]*gap: 14px;/, 'homepage milestone dates and headlines share a stable horizontal baseline');
assert.match(styles, /\.wiki-portal-news-list time \{[\s\S]*font-size: 11px;[\s\S]*line-height: 1\.45;[\s\S]*\}[\s\S]*\.wiki-portal-news-list a \{[\s\S]*font-size: 14px;[\s\S]*line-height: 1\.45;[\s\S]*\}[\s\S]*\.wiki-portal-news-list p \{[\s\S]*font-size: 12\.5px;[\s\S]*line-height: 1\.5;/, 'homepage milestones retain readable date, headline, and detail typography');
assert.match(styles, /\.wiki-portal-directory summary \{[\s\S]*width: min\(100%, 620px\);[\s\S]*margin-inline: auto;/, 'homepage Browse control is centered on the same axis as Latest Updates');
