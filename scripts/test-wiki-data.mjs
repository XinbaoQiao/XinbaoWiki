...
assert.match(styles, /\.wiki-portal-cube-pin \{[\s\S]*position: absolute[\s\S]*top: calc\(var\(--portal-cube-reading-top\) - var\(--portal-cube-pin-height\)\)[\s\S]*right: calc\(100% - var\(--portal-updates-track-offset\) - var\(--portal-cube-reading-width\)\)[\s\S]*left: auto[\s\S]*flex-direction: row-reverse;[\s\S]*min-height: var\(--portal-cube-pin-height\)[\s\S]*border: 0;[\s\S]*border-radius: 0;[\s\S]*background: transparent;[\s\S]*box-shadow: none;[\s\S]*\.wiki-portal-cube-pin-icon \{[\s\S]*filter: drop-shadow[\s\S]*rotate\(-18deg\)[\s\S]*\.wiki-portal-cube-pin\[aria-pressed="true"\] \{\s*background: transparent;\s*box-shadow: none;\s*\}[\s\S]*\.wiki-portal-cube-pin\[aria-pressed="true"\] \.wiki-portal-cube-pin-icon \{[\s\S]*translateY\(10px\)[\s\S]*transition: none;/, "the chrome-free Pin derives both axes from the shared reading-paper geometry, places the visible thumbtack nearest the corner, and presses its actual needle tip into the paper edge");
