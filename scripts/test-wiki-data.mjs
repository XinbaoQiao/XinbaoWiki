...
assert.ok(styles.includes("perspective: 1900px") && styles.includes("transform-style: preserve-3d"), "desktop typographic cube uses a restrained real perspective context");
...
assert.ok(homepagePortal.includes("if (browseView !== 'cube') return") && homepagePortal.includes('if (targetFace === relatedFace') && homepagePortal.includes('scheduleCubeFace(face)') && !homepagePortal.includes("browseView !== 'cube' || activeCubeFace"), "typographic cube can switch directly between intentional face hovers without rescheduling inside one face");
...
assert.match(homepagePortal, /onPointerOut=\{\(event\) => \{[\s\S]*sourceFace[\s\S]*destinationFace[\s\S]*sourceFace && !destinationFace[\s\S]*clearCubeHoverIntent\(\);[\s\S]*setActiveCubeFace\(null\)/, "leaving a face for empty stage space cancels pending hover intent and restores the resting pose");
...
assert.match(styles, /\.wiki-portal-grid-cube\[data-active-face="top"\][\s\S]*\[data-cube-face="right"\] a \{[\s\S]*font-size: 11px[\s\S]*font-weight: 680/, "the head-on reading face slightly enlarges and strengthens link typography without changing resting-face density");
